...
PACKAGE_NAME=$(grep -oP '"name":\s*"\K[^"]+' package.json)
...
# Replace the version in package-lock.json for the main package
sed -i "/\"name\": \"$PACKAGE_NAME\"/,/\"version\": \"$VERSION\"/s/\"version\": \"$VERSION\"/\"version\": \"$NEW_VERSION\"/" package-lock.json
